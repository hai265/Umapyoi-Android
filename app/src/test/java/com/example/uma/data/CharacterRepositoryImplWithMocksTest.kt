...
class CharacterRepositoryImplWithMocksTest {
    ...
    @Before
    fun setup() {
        coJustRun { characterDao.syncCharacters(any()) }
        coJustRun { characterDao.syncCharacters(any()) }
    ...
    fun getAllCharacters_whenDaoHasData_returnsDataFromDao() = runTest {
        ...
        val expected = listOf(
            CharacterBasic(1, 1, "Special Week", "image", "colorMain", "colorSub", isFavorite = false),
            CharacterBasic(2, 2, "Tokai Teio", "image", "colorMain", "colorSub", isFavorite = false)
        )
    ...
    fun sync_fetchesFromNetworkAndSavesToDao() = runTest {
        ...
        coVerify(exactly = 1) { characterDao.syncCharacters(expectedEntitiesToSave) }
    }
    ...
    fun `sync then getAllCharacters, returns newly synced data`() = runTest {
        ...
        coVerify(exactly = 1) { umaApiService.getAllCharacters() }
        coVerify(exactly = 1) { characterDao.syncCharacters(expectedEntitiesAfterSync) }
    }
